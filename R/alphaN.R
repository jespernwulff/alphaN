...
#' @param method Which Bayes factor to calibrate alpha to. The first four
...
#' hypothesis on a prespecified effect size `de`. One of:
#' \itemize{
...
#'   medium effect; use 0.2 for small and 0.8 for large effects (Cohen, 1988).
#' @param nu Degrees of freedom of the prior t distribution for methods "ES"
#'   and "moment". The default, NULL, uses the values recommended by Klauer
#'   et al. (2025): 3 for "ES" and `5 + (q - 1)` for "moment".
#' @param r Scale of the two prior mixture components for method "ES". The
#'   default, NULL, uses the recommendation of Klauer et al. (2025),
#'   \code{r = sqrt((nu - 2)/(nu * q)) * de}, which requires nu > 2 and de > 0;
#'   otherwise supply `r` explicitly.
#' @param q Number of coefficients tested jointly. Only used by methods "ES"
#'   and "moment". The default, 1, is the test of a single coefficient; for
#'   `q > 1` the alpha level is set for the F test of the joint null that all
#'   q coefficients are zero.
#' @param p Number of parameters retained in the model under the null,
#'   including any intercept. Only used by methods "ES" and "moment". The
#'   effective sample size of Klauer et al. (2025) is `n - p`; the default,
#'   0, reproduces the one-sample form, which treats the sample size as
#'   effective. For a regression coefficient in a small sample, setting `p`
#'   to the number of other estimated coefficients (including the intercept)
#'   gives the residual-degrees-of-freedom behavior of the exact regression
#'   case.
...
#'
#' As a special case, setting \code{method = "ES", nu = 1, de = 0} with an
#' explicit scale (e.g. \code{r = 1}) calibrates alpha to the default
#' (Jeffreys-Zellner-Siow type) Bayes factor of Rouder et al. (2009).
#'
#' For effective sample sizes greater than 50,000, methods "ES" and "moment"
#' evaluate the noncentral density ratio in its normal (or chi-square)
#' limit, which is accurate to a fraction of a percent there.
#' @export
#'
#' @examples
...
#' \cr
#' Klauer, K. C., Meyer-Grant, C. G., & Kellen, D. (2025). On Bayes factors for hypothesis tests. Psychonomic Bulletin & Review, 32, 1070–1094. \doi{10.3758/s13423-024-02612-2} \cr
#' \cr
...
#' \cr
#' Rouder, J. N., Speckman, P. L., Sun, D., Morey, R. D., & Iverson, G. (2009). Bayesian t tests for accepting and rejecting the null hypothesis. Psychonomic Bulletin & Review, 16, 225–237. \cr
#' \cr
...
alphaN <- function(n, BF=1, method="JAB", upper = 1, de = 0.5, nu = NULL,
                   r = NULL, q = 1, p = 0) {
  ...
  if (method %in% c("ES", "moment")) {
    return(klauer_alpha(n, BF, method, de = de, nu = nu, r = r, q = q, p = p))
  }

  if (any(q != 1) || any(p != 0)) {
    stop("`q` and `p` apply only to methods \"ES\" and \"moment\".",
         call. = FALSE)
  }
